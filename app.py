...
class calculator:
    ...
    def analyze_image_via_api(self):
        # Convert BGR Canvas to RGB
        img_rgb = cv2.cvtColor(self.imgCanvas, cv2.COLOR_BGR2RGB)
        ...
        # Define your FastAPI server endpoint
        api_url = "http://localhost:8000/analyze-image/"  # Change if hosted remotely

        # Send the image to the FastAPI server
        files = {"file": ("canvas.png", buffer, "image/png")}
